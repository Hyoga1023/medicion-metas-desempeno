...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('https://bht1c1naxikbuzgqpmoo.cleverapps.io/guardar', {
                method: 'POST',
                headers: {
    ...
    document.getElementById('descargar').addEventListener('click', async () => {
        try {
            const response = await fetch('https://bht1c1naxikbuzgqpmoo.cleverapps.io/descargar');
            if (response.ok) {
    ...
    document.getElementById('borrar').addEventListener('click', async () => {
        try {
            ...
            if (result.isConfirmed) {
                const response = await fetch('https://bht1c1naxikbuzgqpmoo.cleverapps.io/borrar-todo', { method: 'DELETE' });
                if (response.ok) {
